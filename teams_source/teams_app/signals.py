...
def create_user_token(sender, instance, **kwargs):
    try:
        ...
        username_hash = hashlib.sha256(
            username
        ).hexdigest()  # Encrypt and get digest value

        user_id = instance.id
        user_token = UserToken(user_id=user_id, username_hash=username_hash)
        user_token.save()

    # Token already exists so do nothing


def signals_imported():
    """
    Done to avoid unused import error.
    """
    print("Signal are imported successfully.")
